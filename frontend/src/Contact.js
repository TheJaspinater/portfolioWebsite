...
const Contact = () => {
    ...
    return (
        <div class="contact-container">
            <div class="form-list">
                {!sent && <div class="contact-form">
                    <label className="text-3xl font-bold mb-5">C O N T A C T</label>
                </div>}
                {!sent && <div class="contact-form">
                    <textarea placeholder="N A M E *" value={name} onChange={(e) => {setName(e.target.value); setNameCSS("form-input-good")}} class={nameCSS}></textarea>
                </div>}
                {!sent && <div class="contact-form">
                    <textarea placeholder="E M A I L *" value={email} onChange={(e) => {setEmail(e.target.value); setEmailCSS("form-input-good")}} class={emailCSS}></textarea>
                </div>}
                {!sent && <div class="contact-form">
                ...
                {!sent && <div class="contact-form">
                    <textarea placeholder="M E S S A G E *" value={message} onChange={(e) => {setMessage(e.target.value); setMessageCSS("form-input-good")}} class={messageCSS} rows="5"></textarea>
                </div>}
                {!sent && <div class="contact-form">
                    <textarea placeholder="V E R I F Y   |   enter a 3 digit number *" value={verify} onChange={(e) => {setVerify(e.target.value); setVerifyCSS("form-input-good")}} class={verifyCSS}></textarea>
                </div>}
                {!sent && <div class="contact-form">
